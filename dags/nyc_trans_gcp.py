...
def _load_weather_data():
    ...
    weather_info = pd.read_csv(gcs_uri)
...
def _transform_citibike_data():
    ...
    df.to_csv('/tmp/t202302-citibike-tripdata.csv',index=False)

    # Define your GCS bucket and destination folder
    bucket_name = 'your_bucket_name'
    blob_name  = 'your_blob_name'
    ...
    blob.upload_from_filename("/tmp/t202302-citibike-tripdata.csv")

transform_citibike_data = PythonOperator(
    task_id="transform_citibike_data", python_callable=_transform_citibike_data, dag=dag
)
...
def _transform_yellowtaxi_data():
    parquet_file_path = '/tmp/yellow_tripdata_2023-02.parquet'
    pf = ParquetFile(parquet_file_path) 
    first_ten_rows = next(pf.iter_batches(batch_size = 2000)) 
    df_yt = pa.Table.from_batches([first_ten_rows]).to_pandas() 
    ...
    df_yt.to_csv('/tmp/tyellow_tripdata_2023-02.csv',index=False)
    ...
    # Upload the CSV file to GCS
    blob = bucket.blob(blob_name)
    blob.upload_from_filename("/tmp/tyellow_tripdata_2023-02.csv")


transform_yellowtaxi_data = PythonOperator(
    task_id="transform_yellowtaxi_data", python_callable=_transform_yellowtaxi_data, dag=dag
)
...
download_citibike >> download_zones >> unzip_citibike >> transform_citibike_data >> load_citibike_into_snowflake
download_zones >> load_zones_into_snowflake
download_yellowtaxi >> transform_yellowtaxi_data >> load_yellowtaxi_into_snowflake
